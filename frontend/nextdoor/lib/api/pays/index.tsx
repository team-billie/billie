...
const handleApiError = (error: any, name: string) => {
  const responseCode = error.response?.data?.responseCode || "UNKNOWN_ERROR";
  const message = error.response?.data?.message || `${name} API 요청 중 오류`;
  const customError = new Error(message) as Error & { code: string };
  customError.code = responseCode;
  throw customError;
};
...
const apiCall = async (endpoint: string, data: any, name: string) => {
  try {
    ...
    throw handleApiError(error, name);
  }
};
